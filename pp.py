...
def wykresy(tab_time, Av_node, Nr_con_no):
    ...
    fig=pl.plot(bins2,counts,'ro',bins2,counts,'-b')
    pl.title('logxlog')
    pl.xlabel("log(k)")
    pl.ylabel("log(P)")
    pl.savefig("logxlog.png")
    pl.cla()
    ...
    fig = pl.plot(range(0,len(t.nodes)), Av_node,'ro')
    pl.axis([0, len(t.nodes)+1, 0, max(Av_node)+1])
    pl.title('rozklad stopni wierzcholkow')
    pl.xlabel("nodes")
    pl.ylabel("liczba polaczen")

    pl.savefig("aktywnosc_n.png")
    pl.cla()
...
def liczymy_polaczenia(active_nodes_list, contacts, time_con, time_cc, start, nowe_active, new_time_con, list):
    ...
    for jj in range(0, len(active_nodes_list)):
        ...
        find_con(contacts, active_nodes_list[jj][len(active_nodes_list[jj]) - 1], time_con[jj][len(time_con[jj]) - 2],
                 time_cc, list)
...
def av_nr_node(C, av_n, Av_node):
    ...
    plik2.write('\n'+nazwa + '\t' + str(len(t.nodes)) + '\t' + str(len(t.tedges)) + '\t' +
                str(t.getObservationLength()) + '\t' + str(15) + '\t' + str(len(time_table_val)) + '\t' + str(av_n))
...
def miary(bet_n, bet_e,C):
    ...
    print('% .4f' % bet_e)
    plik2.write( str('% .4f' % bet_n) + '\t' + str('% .4f' % bet_e) )
    plik2.close()
