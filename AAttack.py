...
for i in range(0, len(t.nodes)):
    ...
    average_num2.append([i, 0])
    average_num3.append([i,0])
...
numerek = 0
for ii in range(4, 9):
    average_num[numerek][1] = float(line[ii][2])
    numerek += 1
eff=[]
for ii in range(len(average_num)):
    eff.append(average_num[ii][1])
eff.sort()
print (average_num)
print(eff)

for jj in range(len(eff)):
    for iii in range(len(average_num)):
        if average_num[iii][1]==eff[jj]:
            average_num2[jj][0]=average_num[iii][0]
            average_num2[jj][1]=eff[jj]
            del average_num[iii]
            break
for jj in range(len(average_num2)):
    if len(average_num2)!=len(average_num3):
        average_num3.append([0,0])
    else:
        average_num3[jj][0]=average_num2[jj][0]
        average_num3[jj][1] = average_num2[jj][1]
for r_p in range(2,11):
    print("attack average node number" + " " + str(float(r_p / 10.0)))
    node_contacts=[]
    #print (node_contacts)
    ...
    if len(average_num3)!=0:
        for c in range(0, len(node_contacts)):

            if (int(node_contacts[c][0]) == int(average_num3[len(average_num3) - 1][0]) or int(
                    node_contacts[c][1]) == int(average_num3[len(average_num3) - 1][0])):
                print("lol")

            else:
                new_node_contacts.append(node_contacts[c])
        del average_num3[len(average_num3) - 1]

        time_table(time_table_val, time_table_nr, new_node_contacts)
        e_po = distance_count2(len(average_num3), new_node_contacts, active_nodes_list1, active_nodes_list_time1)
    ...
    plik2.write('\n')
    plik2.write("a" + '\t' + str(r_p) + '\t' + str(len(t.nodes)) + '\t'
