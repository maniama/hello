...
for i in range(0, len(t.nodes)):
    ...
    efficiency2.append([i, 0])
    efficiency3.append([i,0])
...
numerek = 0
for ii in range(4, 9):
    efficiency[numerek][1] = float(line[ii][1])
    numerek += 1
eff=[]
for ii in range(len(efficiency)):
    eff.append(efficiency[ii][1])
eff.sort()
print (efficiency)
print(eff)

for jj in range(len(eff)):
    for iii in range(len(efficiency)):
        if efficiency[iii][1]==eff[jj]:
            efficiency2[jj][0]=efficiency[iii][0]
            efficiency2[jj][1]=eff[jj]
            del efficiency[iii]
            break
for jj in range(len(efficiency2)):
    if len(efficiency2) != len(efficiency3):
        efficiency3.append([0, 0])
    else:
        efficiency3[jj][0] = efficiency2[jj][0]
        efficiency3[jj][1] = efficiency2[jj][1]
for r_p in range(2,11):
    print("attack efficiency"+" "+str( float(r_p/10.0)))
    node_contacts=[]
    #print (node_contacts)
    ...
    if len(efficiency3)!=0:

        for c in range(0, len(node_contacts)):

            if (int(node_contacts[c][0]) == int(efficiency3[len(efficiency3) - 1][0]) or int(
                    node_contacts[c][1]) == int(efficiency3[len(efficiency3) - 1][0])):
                print("lol")

            else:
                new_node_contacts.append(node_contacts[c])

        del efficiency3[len(efficiency3) - 1]

        time_table(time_table_val, time_table_nr, new_node_contacts)
        e_po = distance_count2(len(efficiency3), new_node_contacts, active_nodes_list1, active_nodes_list_time1)
        # print(new_node_contacts)
        delete_nodes_list(node_contacts)
        del (new_node_contacts)
    else:
        e_po=0
        print("E_po"+str(e_po))
        print(D)
    ...
    plik2.write('\n')
    plik2.write("f" + '\t' + str(r_p) + '\t' + str(len(t.nodes)) + '\t'
