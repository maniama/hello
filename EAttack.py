...
for i in range(0, len(t.nodes)):
    ...
    bet_edge2.append([i, 0])
    bet_edge3.append([i,0])
...
numerek = 0
for ii in range(4, 9):
    bet_edge[numerek][1] = float(line[ii][4])
    numerek += 1
eff=[]
for ii in range(len(bet_edge)):
    eff.append(bet_edge[ii][1])
eff.sort()
print (bet_edge)
print(eff)

for jj in range(len(eff)):
    for iii in range(len(bet_edge)):
        if bet_edge[iii][1]==eff[jj]:
            bet_edge2[jj][0]=bet_edge[iii][0]
            bet_edge2[jj][1]=eff[jj]
            del bet_edge[iii]
            break


    for jj in range(len(bet_edge2)):
        if len(bet_edge2)!=len(bet_edge3):
            bet_edge3.append([0,0])
        else:
            bet_edge3[jj][0]=bet_edge2[jj][0]
            bet_edge3[jj][1] = bet_edge2[jj][1]
for r_p in range(2,11):
    print("attack betweeness edge"+" "+str( float(r_p/10.0)))
    node_contacts=[]
    #print (node_contacts)
    ...
    if len(bet_edge3)!=0:
        for c in range(0, len(node_contacts)):
            ...
            if (int(node_contacts[c][0]) == int(bet_edge3[len(bet_edge3) - 1][0]) or int(
                    node_contacts[c][1]) == int(bet_edge3[len(bet_edge3) - 1][0])):
                print("lol")

            else:
                new_node_contacts.append(node_contacts[c])
        del bet_edge3[len(bet_edge3) - 1]

        time_table(time_table_val, time_table_nr, new_node_contacts)
        e_po = distance_count2(len(bet_edge3), new_node_contacts, active_nodes_list1, active_nodes_list_time1)
        #print(node_contacts)
    ...
    plik2.write('\n')
    plik2.write("e" + '\t' + str(r_p) + '\t' + str(len(t.nodes)) + '\t'
